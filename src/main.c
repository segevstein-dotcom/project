...
//     // --- הגדרת משתנים לצבירת נתונים (כאן היה חסר!) ---
...
//     fprintf(f, "| ID   | Mean (G)| Mean (C) (Err%%)  | Var (G) | Var (C) (Err%%)  | MAE (%%)   | SNR    |\n");
//     fprintf(f, "|------|---------|------------------|---------|------------------|-----------|--------|\n");
...
//         float snr = (mae > 1e-6) ? 20.0f * log10f(sqrtf(gold_var + 1e-10) / mae) : 99.9f;

//         // מעקב אחרי מקרה גרוע ביותר
//         if (mae > max_mae) {
//             max_mae = mae;
...
//         fprintf(f, "| %-4d | %7.4f | %7.4f (%5.1f%%) | %7.4f | %7.4f (%5.1f%%) | %8.2f%% | %6.2f |\n", 
//                 vec_idx, gold_mean, mean_c, mean_err, gold_var, var_c, var_err, mae * 100.0f, snr);
//     }

//     // --- סיכום סופי ---
//     fprintf(f, "\n========================================\n");
//     fprintf(f, "FINAL SUMMARY Statistics (Global Average)\n");
//     fprintf(f, "========================================\n");
//     fprintf(f, "Total Vectors Processed:  %d\n", NUM_VECTORS);
//     fprintf(f, "Average Mean Error:       %.2f%%\n", total_mean_err_pct / NUM_VECTORS);
//     fprintf(f, "Average Variance Error:   %.2f%%\n", total_var_err_pct / NUM_VECTORS);
//     fprintf(f, "Average Global MAE:       %.4f%%\n", (global_total_mae / NUM_VECTORS) * 100.0f);
//     fprintf(f, "Maximum MAE Encountered:  %.4f%% (Vector #%d)\n", max_mae * 100.0f, max_mae_vec_idx);
//     fprintf(f, "========================================\n");

//     fclose(f);
//     printf("Report generated successfully: final_report.txt\n");
//     return 0;
// }
